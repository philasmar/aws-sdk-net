...
namespace Amazon.Braket.Model
{
    ...
    internal sealed partial class SearchQuantumTasksPaginator : IPaginator<SearchQuantumTasksResponse>, ISearchQuantumTasksPaginator
    {
...
#if BCL
        IEnumerable<SearchQuantumTasksResponse> IPaginator<SearchQuantumTasksResponse>.Paginate()
        {
            ...
            SearchQuantumTasksResponse response;
            do
            {
                _request.NextToken = nextToken;
                response = _client.SearchQuantumTasks(_request);
        ...
        async IAsyncEnumerable<SearchQuantumTasksResponse> IPaginator<SearchQuantumTasksResponse>.PaginateAsync(CancellationToken cancellationToken = default)
        {
            if (Interlocked.Exchange(ref _isPaginatorInUse, 1) != 0)
            {
                throw new System.InvalidOperationException("Paginator has already been consumed and cannot be reused. Please create a new instance.");
            ...
            {
                _request.NextToken = nextToken;
                response = await _client.SearchQuantumTasksAsync(_request, cancellationToken).ConfigureAwait(false);
                nextToken = response.NextToken;
