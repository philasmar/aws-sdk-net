...
namespace Amazon.Chime.Model
{
    ...
    internal sealed partial class ListChannelMembershipsPaginator : IPaginator<ListChannelMembershipsResponse>, IListChannelMembershipsPaginator
    {
...
#if BCL
        IEnumerable<ListChannelMembershipsResponse> IPaginator<ListChannelMembershipsResponse>.Paginate()
        {
            ...
            ListChannelMembershipsResponse response;
            do
            {
                _request.NextToken = nextToken;
                response = _client.ListChannelMemberships(_request);
        ...
        async IAsyncEnumerable<ListChannelMembershipsResponse> IPaginator<ListChannelMembershipsResponse>.PaginateAsync(CancellationToken cancellationToken = default)
        {
            if (Interlocked.Exchange(ref _isPaginatorInUse, 1) != 0)
            {
                throw new System.InvalidOperationException("Paginator has already been consumed and cannot be reused. Please create a new instance.");
            ...
            {
                _request.NextToken = nextToken;
                response = await _client.ListChannelMembershipsAsync(_request, cancellationToken).ConfigureAwait(false);
                nextToken = response.NextToken;
